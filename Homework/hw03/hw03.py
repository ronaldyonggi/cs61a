...
def pingpong(n):
    ...
    def pingpong(n):
        ...
        # The argument 'x' is the incrementer or decrementer.
        # To understand the code, look at the Normal Recursive Case
        def helper(total, index, x):
            ...
            # Normal Recursive Case: For every recursive call, increment the total by x (which we'll
            # call 1). But if has_seven is True, then we decrement total by x and negate x so that the 
            # Normal Recursive Call will be calling -x
            return helper(total + x, index + 1, x)
        return helper(1, 1, 1)
